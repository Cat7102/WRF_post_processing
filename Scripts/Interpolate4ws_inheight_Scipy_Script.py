...
#计算沿着高度进行的插值
def interpolate_wind_inheight(ncfile,time_num,function_type,input_lon,input_lat,speedup_index):
    ...
    if speedup_index==1:#这个表示不进行加速计算
        lon_x,lat_x=lon,lat
        ua_s = to_np(getvar(ncfile, 'ua', time_num))
        va_s = to_np(getvar(ncfile, 'va', time_num))
        wa_s = to_np(getvar(ncfile, 'wa', time_num))
        slice_num = [0, lon.shape[0], 0, lon.shape[1]]
    else:
        ...
        ua_s = to_np(getvar(ncfile, 'ua', time_num))[:, line_start:line_end, point_start:point_end]
        va_s = to_np(getvar(ncfile, 'va', time_num))[:, line_start:line_end, point_start:point_end]
        wa_s = to_np(getvar(ncfile, 'wa', time_num))[:, line_start:line_end, point_start:point_end]
        ##################################################################
    ...
    for i in range(ua_s.shape[0]):
        nearest_point_ua.append(
            to_np(getvar(ncfile, 'ua', time_num))[i, input_line_index, input_point_index])
        nearest_point_va.append(
            to_np(getvar(ncfile, 'va', time_num))[i, input_line_index, input_point_index])
        nearest_point_wa.append(
            to_np(getvar(ncfile, 'wa', time_num))[i, input_line_index, input_point_index])
    height2earth=get_ncfile_height2earth_vague(ncfile)
    time_list=get_ncfile_time(ncfile)
    # 写入xlsx
    write_to_xlsx(height2earth,windspeed_interpolate_ua,windspeed_interpolate_va,windspeed_interpolate_wa,windspeed_interpolate,
                  winddirection_interpolate,nearest_point_ua,nearest_point_va,nearest_point_wa,
                  input_line_index,input_point_index,lon,lat,time_list[time_num],speedup_index=speedup_index,slice_num=slice_num)
...
def write_to_xlsx(coloum_1,ua_list,va_list,wa_list,ws_list,wdir_list,nearest_ua_list,nearest_va_list,nearest_wa_list,
                  input_line_index,input_point_index,lon,lat,time,speedup_index=None,slice_num=None):
    # 创建xlsx
    workbook = openpyxl.Workbook()
    #写入风速插值
    worksheet = workbook.create_sheet()
    worksheet.title = '风速插值'
    worksheet.cell(1,1,'高度(m)') #这里可以进行修改，第一列是什么就写什么标题
    ...
    workbook.save(filename="wrf_windspeed_inheight"+".xlsx")
    print("xlsx文件写入完毕")
